...
RSpec.describe 'Api::V1 security and validation boundaries', type: :request do
  ...
  describe 'role permissions' do
    it 'allows staff to update their own client but does not expose another user client' do
      other_staff = create(:user, :staff, account: account)
      ...
      other_staff_client = create(
        :client,
        account: account,
        user: other_staff
      )

      patch "/api/v1/clients/#{staff_client.id}",
      ...
      patch "/api/v1/clients/#{other_staff_client.id}",
            headers: auth_headers(staff),
            params: {
    ...
    it 'allows an owner to update the account' do
      patch '/api/v1/account',
            headers: auth_headers(owner),
            params: {
              account: { business_name: 'Updated Acme' },
            }
    ...
    it 'prevents read-only users from creating, updating, or deleting records' do
      operations = [
        [
          :post,
          '/api/v1/clients',
          { client: { first_name: 'Blocked', last_name: 'Client' } },
          'Read-only users cannot make changes.',
        ],
        [
          :patch,
          "/api/v1/clients/#{client.id}",
          { client: { phone: '555-0001' } },
          'Read-only users cannot make changes.',
        ],
        [
          :delete,
        ...
        [
          :post,
          '/api/v1/services',
          { service: { title: 'Blocked Service', price: 10 } },
          'Read-only users cannot make changes.',
        ],
        [
          :patch,
          "/api/v1/services/#{service.id}",
          { service: { title: 'Blocked Service' } },
          'Read-only users cannot make changes.',
        ],
        [
          :delete,
        ...
        [
          :patch,
          '/api/v1/account',
          { account: { business_name: 'Blocked Account' } },
          'Only account owners can do that.',
        ],
        [
          :post,
        ...
        [
          :patch,
          "/api/v1/users/#{read_only_user.id}",
  ...
  describe 'account isolation' do
    ...
    it 'does not expose records belonging to another account' do
      [
        ...
        expect(response).to have_http_status(:not_found),
                            "expected #{path} to be account-isolated"
        expect(json.dig('error', 'code')).to eq('not_found')
      end
    end
  end
